...
public class Rigid_Bunny_by_Shape_Matching : MonoBehaviour
{
	...
	void Collision_Impulse(Vector3 P, Vector3 N,int i){
		Vector3 px = Y[i]-P;
		if(Vector3.Dot(px,N)<0){
			Debug.Log("可爱的");
			Y[i] += (-1.0f)*Vector3.Dot(px,N)*N;
			/*
			Vector3 v_n = Vector3.Dot(V[i],N)*N;
			Vector3 v_t = V[i] - v_n;
	...
	void Collision(float inv_dt)
	{
		...
		for(int i=0; i<V.Length; i++){
			Collision_Impulse(new Vector3(0, 0.01f, 0), new Vector3(0, 1, 0),i);
			Collision_Impulse(new Vector3(2, 0, 0), new Vector3(-1, 0, 0),i);
		}
	}
